...
local Settings = {
    Aimbot = {
        Enabled = false,
        ShowFOV = false,
        FOV_Radius = 120, -- حجم دائرة المنتصف
        IsAiming = false  -- متغير للتحقق من الضغط على الزر الأيمن
    },
    ESP = {
        Enabled = false,
        Box = true,
...
ScreenGui.Name = "ProHub_V4"
ScreenGui.ResetOnSpawn = false
ScreenGui.IgnoreGuiInset = true -- مهم جداً لجعل الدائرة والمربعات دقيقة مع الكاميرا
...
-- إضافة الأزرار
CreateToggle("AimbotToggle", "تفعيل Aimbot (كليك يمين)", 50, function(state)
...
-- زر الانتقال الآني (زر عادي وليس Toggle)
...
-- ==========================================
-- 4. دوال الايم بوت (المنتصف والثبات الخارق)
-- ==========================================
UserInputService.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton2 then Settings.Aimbot.IsAiming = true end
end)
UserInputService.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton2 then Settings.Aimbot.IsAiming = false end
end)

local function GetClosestPlayerToCenter()
    local closestPlayer = nil
    local shortestDist = Settings.Aimbot.FOV_Radius
    local screenCenter = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2) -- مركز الشاشة
...
-- تثبيت خارق بكسر الكاميرا الأساسية
RunService:BindToRenderStep("SuperAimbot", Enum.RenderPriority.Camera.Value + 1, function()
    if Settings.Aimbot.Enabled and Settings.Aimbot.IsAiming then
...
-- ==========================================
-- 5. الـ ESP فائق الدقة (Hitbox Fix)
-- ==========================================
local ESP_Objects = {}
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer then
            ...
            if char and char:FindFirstChild("HumanoidRootPart") and char:FindFirstChild("Head") and char.Humanoid.Health > 0 then
                -- الحساب الدقيق: أعلى نقطة في الرأس، وأسفل نقطة في القدم
                ...
                if onScreen1 or onScreen2 then
                    ...
                    local width = height / 2 -- تناسب روبلوكس الطبيعي
                    ...
                    -- رسم الخط (Tracer) من أعلى الشاشة
